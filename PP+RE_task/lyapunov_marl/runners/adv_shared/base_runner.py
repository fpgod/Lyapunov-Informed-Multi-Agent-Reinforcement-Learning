...
from utils.shared_buffer_advinf import SharedReplayBuffer_advinf
from algorithms.fgsm import FGSM
from algorithms.jsma import JSMA
...
class Runner(object):
    ...
    def __init__(self, config):
        ...
        wandb.init( project='old_robots_adv', 
                    name='Algo {}_lypnov Exp {}'.format(self.algorithm_name,
                                                 self.experiment_name))
        self.writter_wandb = wandb

        if self.all_args.algorithm_name == "happo":
            from algorithms.happo_policy import HAPPO_Policy as VictimPolicy
        elif self.all_args.algorithm_name == "hatrpo":
            from algorithms.hatrpo_policy import HATRPO_Policy as VictimPolicy
        ...
        if self.all_args.adv_algorithm_name == "mappo_iclr" or \
            ...
            from algorithms.advinf_trainer import advinf as TrainAlgo
            from algorithms.advinf_policy import advinf_Policy as Policy
        ...
        elif self.all_args.adv_algorithm_name == "mappo_jsma":
            self.attack = JSMA(self.all_args, self.victim_policy, self.policy, device = self.device)
        ...
        if self.all_args.adv_algorithm_name == 'mappo_iclr' or \
            ...
            self.buffer = SharedReplayBuffer_advinf(self.all_args,
                                            self.num_agents,
                                            self.num_adv_agents,
                                            self.envs.observation_space[0],
                                            share_observation_space,
    ...
    def log_train(self, train_infos, total_num_steps):
        ...
        for k, v in train_infos.items():
            ...
            self.writter_wandb.log({"train/" + k: v}, step=total_num_steps)
    ...
    def log_env(self, env_infos, total_num_steps):
        ...
        for k, v in env_infos.items():
            ...
            self.writter_wandb.log({"test/" + k: np.mean(v)}, step=total_num_steps)
