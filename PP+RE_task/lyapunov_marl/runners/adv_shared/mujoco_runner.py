...
class MujocoRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            # decay std coef
            x_coef_fin = self.config['all_args'].std_x_coef
            y_coef_fin = self.config['all_args'].std_y_coef
            self.policy.actor.act.action_out.std_x_coef = 1 - (1 - x_coef_fin) * min(episode, 50) / 50
            self.policy.actor.act.action_out.std_y_coef = 0.5 - (0.5 - y_coef_fin) * min(episode, 50) / 50
            print("Current std coef: {:.3f}, {:.3f}".format(self.policy.actor.act.action_out.std_x_coef, self.policy.actor.act.action_out.std_y_coef))
            ...
            for step in range(self.episode_length):
                ...
                obs, share_obs, rewards, rewards_lyn,dones, infos, available_actions = self.envs.step(actions_use)
                ...
                if self.config['all_args'].adv_algorithm_name == 'mappo_advinf':
                    maa_pred, maa_log_probs, rnn_states_maa, oracle_actions, oracle_action_log_probs, \
                        rnn_states_oracle, oracle_values, rnn_oracle_states_critic, adv_action_prob = self.collect_advinf(step, actions_use, available_actions)
                ...
                if self.all_args.adv_algorithm_name == "mappo_iclr" or \
                    self.all_args.adv_algorithm_name == "mappo_fgsm" or \
                    self.all_args.adv_algorithm_name == "mappo_jsma":
                    data = obs, share_obs, rewards_lyn, dones, infos, \
                       values, actions, action_log_probs, \
                       rnn_states, rnn_states_critic
                    self.insert(data)
            ...
            if self.all_args.adv_algorithm_name == 'mappo_advinf':
                advinf_rewards = self.trainer.get_target_influence_reward_continuous(self.buffer)
    ...
    @torch.no_grad()
    def collect_advinf(self, step, action_use, available_actions=None):
    ...
    def insert_advinf(self, data):
        ...
        # print(share_obs.shape, obs.shape, rnn_states.shape, rnn_states_critic.shape, actions.shape, action_log_probs.shape, values.shape, 
        # adv_rewards.shape, masks.shape, bad_masks.shape, active_masks.shape, available_actions.shape)
    ...
    def build_adv_rewards(self, rewards, infos):
        ...
        for t1 in range(self.n_rollout_threads):
            for t2 in range(self.num_agents):
                adv_rewards[t1, t2, 0] = - infos[t1]["reward_run"] + self.all_args.reward_ctrl_param * infos[t1]["reward_ctrl"]
    ...
    def eval(self, total_num_steps):
        ...
        while True:
            ...
            for eval_i in range(self.n_eval_rollout_threads):
                ...
                eval_episode_reward_run[eval_i] += eval_infos[0]["reward_run"]
                eval_episode_reward_ctrl[eval_i] += eval_infos[0]["reward_ctrl"]
            ...
            if eval_episode >= self.all_args.eval_episodes:
                ...
                if self.all_args.use_nni:
                    import nni
                    nni.report_intermediate_result(reward_run / eval_episode)
