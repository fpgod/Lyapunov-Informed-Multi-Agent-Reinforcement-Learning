...
class MujocoRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                obs, share_obs, rewards, rewards_lyn,dones, infos, available_actions = self.envs.step(actions)
            ...
            if episode % self.log_interval == 0:
                ...
                if len(done_episodes_rewards) > 0:
                    ...
                    self.writter.add_scalar("train/aver_episode_rewards", aver_episode_rewards,
                                             total_num_steps)
                    self.writter_wandb.log({"train/aver_episode_rewards": aver_episode_rewards}, step=total_num_steps)
    ...
    def eval(self, total_num_steps):
        ...
        while True:
            ...
            # Obser reward and next obs
            eval_obs, eval_share_obs, eval_rewards, eval_rewards_lyn, eval_dones, eval_infos, _ = self.eval_envs.step(
                eval_actions)
            for eval_i in range(self.n_eval_rollout_threads):
