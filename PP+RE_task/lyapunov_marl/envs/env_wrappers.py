...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        cmd, data = remote.recv()
        if cmd == 'step':
            ob, s_ob, reward,reward_lyn, done, info, available_actions = env.step(data)
            if 'bool' in done.__class__.__name__:
                if done:
            ...
            remote.send((ob, s_ob, reward,reward_lyn, done, info, available_actions))
        elif cmd == 'reset':
            ob, s_ob, available_actions = env.reset()
...
class ShareSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        elif self.action_space[0].__class__.__name__ == "Box":
            self.n_actions = action_space[0].shape[0]
    ...
    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        obs, share_obs, rews, rews_lyn,dones, infos, available_actions = zip(*results)
        return np.stack(obs), np.stack(share_obs), np.stack(rews), np.stack(rews_lyn), np.stack(dones), infos, np.stack(available_actions)
...
class ShareDummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [env.step(a) for (a, env) in zip(self.actions, self.envs)]
        obs, share_obs, rews, rews_lyn,dones, infos, available_actions = map(
        ...
        return obs, share_obs, rews, rews_lyn, dones, infos, available_actions
