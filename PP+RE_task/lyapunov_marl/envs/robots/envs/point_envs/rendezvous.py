...
class RendezvousEnv(gym.Env, EzPickle):
    ...
    def step(self, actions):
        ...
        rewards, reward_run, reward_ctrl,rewards_lyn = self.get_reward(actions)
        ...
        return next_obs, self.get_state(), rewards, rewards_lyn, [done] * self.nr_agents, info, None
    ...
    def get_reward(self, actions):
        ...
        lyn_rew = (dist_rew - self.dist_rew_pre)*100
        if lyn_rew > 10.0:
            lyn_rew = 0.0
        if dist_rew<0.05:
            lyn_rew -= 2
        self.dist_rew_pre = dist_rew
        lyn_rew = - lyn_rew - action_pen
        lyn_rew = np.ones((self.nr_agents, 1)) * lyn_rew

    

        return r, - dist_rew, - action_pen , lyn_rew
