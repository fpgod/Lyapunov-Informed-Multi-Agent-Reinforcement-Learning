...
class advinf():
    ...
    def train_maa(self, sample):
        ...
        if self.policy.action_type == 'Discrete':
            ...
            if self.args.maa_mode == 'mean':
                maa_mean = self.policy.get_maa_mean(share_obs_batch, actions_all_batch, rnn_states_maa_batch, masks_batch, available_actions_all_next_batch)
                actions_all_next_batch = check(actions_all_next_batch).to(**self.tpdv)
                maa_mean = maa_mean.view(maa_mean.shape[0], -1)
                actions_all_next_batch = actions_all_next_batch.view(actions_all_next_batch.shape[0], -1)
                loss_maa = ((maa_mean - actions_all_next_batch) ** 2).mean()
            elif self.args.maa_mode == 'prob':
                actions_all_next_batch = check(actions_all_next_batch).to(**self.tpdv)
    ...
    @torch.no_grad()
    def get_target_influence_reward(self, buffer):
        maa_all = self.policy.evaluate_maa_probs(buffer.share_obs[:-1], buffer.actions, buffer.rnn_states_maa[:-1], buffer.masks[:-1], buffer.available_actions_all, buffer.active_masks, buffer.adv_actions_prob)
        oracle_actions = check(buffer.oracle_actions).to(**self.tpdv)
        oracle_actions_onehot = torch.nn.functional.one_hot(oracle_actions.long(), num_classes=self.args.n_actions).squeeze()
        if self.args.position_mask_active:
            position_mask = buffer.active_masks_all[:-1].copy()
            position_mask = position_mask.swapaxes(2, 3)
            position_mask[:, :, :, 0] = 0
        else:
            position_mask = self.get_position_mask(buffer.obs)[:-1]
        position_mask = check(position_mask).to(**self.tpdv)
        maa_out = torch.einsum('ijkl, ijmk->ijkl', maa_all, position_mask)
        oracle_out = torch.einsum('ijkl, ijmk->ijkl', oracle_actions_onehot, position_mask)

        if self.args.use_oracle_prob:
            sensitivity = -(maa_out * oracle_out).sum(dim=-1)
        else:
            sensitivity = torch.norm(maa_out - oracle_out, p=1, dim=-1)
        advinf = torch.sum(sensitivity, dim=-1)
        advinf = advinf.unsqueeze(-1).unsqueeze(-1)
        return advinf
    ...
    def get_target_influence_reward_continuous(self, buffer):
        ...
        advinf = torch.sum(torch.sum(maa_all, dim=-1), dim=-1)
        advinf = advinf.unsqueeze(-1).unsqueeze(-1)
        return advinf
    ...
    def train(self, buffer, update_actor=True):
        ...
        train_info['average_step_advinf_rewards'] = 0
